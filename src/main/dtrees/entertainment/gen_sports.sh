...
echo "# wikipedia List_of_professional_sports_teams_in_the_United_States_and_Canada" > $OUT
echo "#\$regex" >> $OUT
echo "#\$dups" >> $OUT
echo "#!unknown,exact,low,high" >> $OUT


curl "$URL" | grep -E "<h3>|<b>" | grep "/wiki/" | sed "s/<h3>/H3:/" | sed "s/<[^>]*>//g" | sed "s/ \[edit\]//" | gawk '
BEGIN{
